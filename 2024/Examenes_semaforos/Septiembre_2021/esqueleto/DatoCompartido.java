...
public class DatoCompartido {
	...
	Semaphore finProcesam = new Semaphore(0);
	Semaphore empezarProces = new Semaphore(0);
	Semaphore procesando = new Semaphore(1);
	Semaphore [] procesos;
	...
	public DatoCompartido(int nProcesadores) {
		...
		procesos = new Semaphore[nProcesadores];
		for (int i = 0; i < nProcesadores; i++) {
			procesos[i] = new Semaphore(0);
		}
	}

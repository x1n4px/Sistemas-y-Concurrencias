...
public class DatoCompartido {
	...
	public DatoCompartido(int nProcesadores) {
		...
		for (int i = 0; i < this.procesadoOno.length; i++) {
			procesadoOno[i] = false;
		}
	}
	...
	public synchronized int generaDato(int d) throws InterruptedException {
		...
		System.out.println("Numero de procesos pendientes: "+ procPend);
		datoGenerado = true;
		notifyAll();
		while(procPend > 0){
			wait();
		...
		for(int i=0; i<procesadoOno.length; i++) {
			procesadoOno[i] = false;
		}
		notifyAll();
		return dato;
